<template>
  <div>
    <Modal v-model="previewEnable" width="60">
      <div v-html="previewItem" class="preview-item"></div>
      <div slot="footer">
        <Button type="error" size="large" @click="previewEnable = false">删除</Button>
        <Button type="info" size="large" @click="previewEnable = false">确认</Button>
      </div>
    </Modal>
    <div class="message-main">
      <Card class="message-card" v-for="(item, index) in items" v-bind:key="index" >
        <div class="message-card-content" v-html="item" @click="previewMessage(index)"></div>
      </Card>
    </div>
    <div class="message-main">
      <Card class="message-add-card">
        <router-link to="/admin/message/edit/1">
          <div><Icon type="android-add"></Icon></div>
        </router-link>
      </Card>
    </div>
  </div>
...
export default {
  ...
  methods: {
    previewMessage (index) {
      ...
      this.previewItem = this.items[index];
    }
  }
}
</script>
